# app/AGENTS.md — Data Ingestion Pipeline 전용 규칙

이 문서는 Data Ingestion Pipeline 영역(`app/`, `tests/`)에서만 적용되는 Codex 규칙을 정의한다.
작업 시 루트 `AGENTS.md`의 공통 규칙을 먼저 적용하고, 이 문서의 규칙을 추가로 따른다.
기존 Claude Code 사용자를 위해 `app/CLAUDE.md`도 함께 유지한다.
...
chunker·embedder·schemas·storage·adapters 는 RAG 레포에서 복사한 공유 자산이다. 공통 계약 변경 시
RAG 레포와의 분기 영향을 먼저 설명한다(루트 `AGENTS.md` 참조).
...
- 각 단계는 RabbitMQ 큐(Ingestion / Attachment / Chunking / Embedding)로 연결되며, Worker는 단일 책임을 갖는다.
- 한 단계가 다른 단계의 책임(예: 추출이 청킹을, 청킹이 임베딩을)을 침범하지 않는다.
- 질의/응답(Query) 단계는 별도 저장소(`../rag`)의 책임이며 본 레포에서 다루지 않는다.

## 3. 보안·정확성 (절대 규칙)

- ACL을 색인 시점에 정확히 적재한다. 페이지·청크 payload에 `allowed_groups`/`allowed_users`를 동반 적재해야 Query 단계의 ACL Pre-filtering이 동작한다.
...
- Secret·API Key·Confluence `access_token`은 코드·로그·테스트 픽스처·메시지 페이로드에 포함하지 않는다. 설정은 `app/config.py`에서 환경 변수로 주입한다.
...
- Delta Sync는 `version`/`updatedAt` 비교로 변경·삭제 페이지만 처리한다.
...
- 수집/청킹/임베딩 잡의 진행·결과는 `ingestion_jobs`(MongoDB)에 단계별 상태로 기록한다.
- Worker 커스텀 메트릭(잡 카운터·지연·실패율)은 Prometheus로 관측한다.
