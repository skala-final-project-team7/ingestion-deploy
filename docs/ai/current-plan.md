...
3. `app/api/deps.py`와 `app/ingestion/workers/ingest_job_main.py` DI seam 정리
...
- [ ] 단위: internal 전용 client에서 헤더 주입/분기 분리 단위 테스트
  (`/internal` vs 공개 API, adminUserId/헤더 없음/헤더 포함).
- [ ] 단위: `_resolve_runtime_credentials` 상태코드별 분기(200/400/401/403/404) 경로 테스트.
- [ ] 단위: 401에서 누락/미스매치 메시지 분리와 로그 마스킹 정책 테스트.
- [ ] 통합: 키 일치 시 200 응답(`accessToken/cloudId/siteUrl/expiresAt`) 수신 후 수집 흐름 연동 테스트.
- [ ] 통합: 키 누락/오류/미스매치 시 401, 400, 403, 404 응답 재현 및 job 실패/경고 정책 검증.
- [ ] 통합: `siteUrl` 반영한 Confluence API URL 구성 경로(`.../ex/confluence/{cloudId}/...`) 검증.
- [ ] 통합: `credential_lookup` 미주입/빈 값 시 fail-fast 또는 경고 정책 검증(운영/PoC mode 분기).
...
- `docs/architecture.md` 및 `docs/ai/working-log.md`(또는 운영 runbook) 갱신 반영.
